...
namespace ncore
{
    ...
    // extern(2) void DrawSpriteScaled(u16 spriteId, u16 x, u16 y, u16 w, u16 h);
    // extern(3) void DrawText(u8 fontId, u8 fontSize, const u8* text, u16 x, u16 y, u32 color);
    // extern(4) void DrawDate(u8 fontId, u8 fontSize, u16 x, u16 y, u32 color);
    // extern(5) void DrawTime(u8 fontId, u8 fontSize, u16 x, u16 y, u32 color);
    // extern(6) void DrawValue(u8 fontId, u8 fontSize, i32 value, u8 unit, u16 x, u16 y, u32 color);
    //
    // extern(7) 
    ...
    void extern_host_fn(void* host_context, vm_t* vm, u32 import_id)
    {
        ...
        switch (import_id)
        {
            ...
            case EXTERN_DRAW_SPRITE_SCALED:
                {
                    ...
                    ngui::draw_sprite(spriteId, x, y, w, h);
                }
                break;
            ...
            case EXTERN_DRAW_DATE:
                {
                    const u16 year        = get_year(app_data->m_house_meta);
                    const u8  month       = get_month(app_data->m_house_meta);
                    const u8  day         = get_day(app_data->m_house_meta);
                    const u8  day_of_week = get_day_of_week(app_data->m_house_meta);
            ...
            case EXTERN_DRAW_TIME:
                {
                    const u8 hour   = get_hour(app_data->m_house_meta);
                    const u8 minute = get_minute(app_data->m_house_meta);
                    const u8 second = get_second(app_data->m_house_meta);
