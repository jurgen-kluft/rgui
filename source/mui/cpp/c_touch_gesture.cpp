...
namespace ncore
{
    namespace nmui
    {
        ...
        static gesture_type_t s_evaluate_swipe(touch_gesture_t& tg, i32 dx, i32 dy, i32 fingers)
        {
        ...
        gesture_type_t update_touch_gesture(touch_gesture_t& tg, bool p1_present, touch_point_t const& p1, bool p2_present, touch_point_t const& p2)
        {
            ...
            {
                tg.m_touchStartTime     = ntimer::millis();
                tg.m_startP1            = p1;
                tg.m_startP2            = p2;
            ...
            if (!p1_present && !p2_present && tg.m_isTracking)
            {
                const u64 duration = ntimer::millis() - tg.m_touchStartTime;
                tg.m_isTracking    = false;
                ...
                {
                    if (ntimer::millis() - tg.m_lastTapTime < tg.m_cfg.m_doubleTapGapMs)
                    {
                    ...
                    else
                    {
                        ...
                        tg.m_lastTapTime = ntimer::millis();
                    }
