#ifndef __RMUI_TOUCH_GESTURE_H__
#define __RMUI_TOUCH_GESTURE_H__
...
namespace ncore
{
    namespace nmui
    {
        ...
        const char* to_string(gesture_type_t gesture);  // Utility to get a text string of the gesture name

        static inline bool is_tap(gesture_type_t gt) { return gt == GT_TAP || gt == GT_DOUBLE_TAP; }
        static inline bool is_double_tap(gesture_type_t gt) { return gt == GT_DOUBLE_TAP; }
        static inline bool is_swipe(gesture_type_t gt) { return (gt & (GT_SWIPE_ONE_FINGER | GT_SWIPE_TWO_FINGER)) != 0; }
        static inline bool is_two_finger(gesture_type_t gt) { return (gt & GT_SWIPE_TWO_FINGER) != 0; }
        static inline bool is_swipe_up(gesture_type_t gt) { return (gt & GT_DIR_UP) != 0 && is_swipe(gt); }
        static inline bool is_swipe_down(gesture_type_t gt) { return (gt & GT_DIR_DOWN) != 0 && is_swipe(gt); }
        static inline bool is_swipe_left(gesture_type_t gt) { return (gt & GT_DIR_LEFT) != 0 && is_swipe(gt); }
        static inline bool is_swipe_right(gesture_type_t gt) { return (gt & GT_DIR_RIGHT) != 0 && is_swipe(gt); }
        ...
        gesture_type_t update_touch_gesture(touch_gesture_t& tg, bool p1_present, touch_point_t const& p1, bool p2_present, touch_point_t const& p2);
...
#endif  // __RMUI_TOUCH_GESTURE_H__